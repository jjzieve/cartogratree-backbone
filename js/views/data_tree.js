...
define([
  ...
  'goog!maps,3,other_params:sensor=false'
		...
		var DataTreeView = Backbone.View.extend({
			el: "#data_tree",
			appendChild: function(parent_index,child_name,sql){
					$.get(
						this.fusion_table_url+sql+this.fusion_table_key,
						function(data){
							$.each(data.rows, function(index,value){
								var node = $("#data_tree").treetable("node", parent_index);
								$('#data_tree').treetable("loadBranch",node,'<tr data-tt-parent-id="'+parent_index+'" data-tt-id="'+parent_index+'-'+index+'"><td>'+value[0]+'</td></tr>');
							});
						}
					);
				},
